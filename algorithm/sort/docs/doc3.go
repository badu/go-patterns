...
// A decision tree is a full binary tree that represents the comparisons between elements that are performed by
// a particular sorting algorithm operating on an input of a given size.
...
// The left subtree then dictates subsequent comparisons for ai <= aj, and
...
// 1) Each of the n! permutations on n elements must appear as one of the leaves of the decision tree for
//    the sorting algorithm to sort properly.
// 2) Let x be the maximum number of comparisons in a sorting algorithm.
//    The maximum height of the decision tree would be x. A tree with maximum height x has at most 2^x leaves.
//
...
//      x = Ω(n Log2 n)
//
// Therefore, any comparison based sorting algorithm must make at least n Log2 n comparisons to sort the input array,
